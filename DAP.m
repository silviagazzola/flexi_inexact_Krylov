function [X, Enrm, Rnrm] = DAP(A, b, MaxIter, xexact, x0, tol, rest, pnorm)
% DAP inexact/flexible solver for p-norm data fitting problems
%
...
p = pnorm;
tolX = 1e-3; % tolerance on the weights
...
for k=1:MaxIter
    ...
    V(:, k+1) = v;
    %
    rhsproj = T(1,1)*rhs(1:k);
    Aproj = T(1:k,:)*M(1:k+1,1:k);
    s = Aproj\rhsproj;
    x = x0 + V(:,1:k)*s;
    X(:,k) = x;
